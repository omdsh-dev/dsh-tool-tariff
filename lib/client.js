...
//
// 数据通道：浏览器侧无 API Key，余额与时段配置经专用 /tariff RPC 通道调
// host half（src/index.ts 里 connection.rpc.handle('/tariff') 注册的
// status、balance），本文件的 apply(ctx) 里用 ctx.get('connection') 取句柄。
// 注意：不要走 /api 通道——那是 DSH 网关（Typert Remote）独占的共享通道。
//
...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const plugin = {
      ...
      inject: ["slots"],
      /**
       * 注册顶部栏右侧状态徽标。数据经 /tariff RPC 从 host half 获取：
      ...
      apply(ctx) {
        ...
        const api = connection === undefined || connection === null
          ...
          : {
              ...
              call: (method, args) => connection.rpc.call("/tariff", method, args === undefined ? null : args)
            };
