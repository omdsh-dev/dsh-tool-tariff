...
export function apply(ctx, rawConfig = {}) {
    ...
    // ---- Web UI 数据通道（browser half 经 /api RPC 读取） ----
    // connection 服务只存在于 web 类 profile（headless 没有）；用 ctx.inject
    // 延迟挂载：服务可用时才注册拦截器，服务消失时自动卸载。
    ctx.inject(['connection'], (scopedCtx) => {
        ...
        const dispose = connection.rpc.intercept('/api', (endpoint) => endpoint === 'tariff/status' || endpoint === 'tariff/balance', handler, { authority: 'loopback' });
        scopedCtx.effect(() => dispose, 'dsh-tool-tariff: /api RPC 通道');
    });
    // ---- 峰时切换提醒 ----
