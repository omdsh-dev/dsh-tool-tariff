...
export function toMinutes(hhmm) {
    ...
    if (match === null)
        throw new Error(`tariff: 非法时间 "${hhmm}"，应为 HH:MM（如 08:50）`);
 ...
 */
export function classifyTime(hhmm) {
    ...
    if (segment === undefined)
        throw new Error(`tariff: 时间 ${hhmm} 不在任何时段内`);
    return segment;
}
/**
 * 计算某时钟时间下的峰谷状态。
 ...
 */
export function tariffStatus(hhmm) {
    const minutes = toMinutes(hhmm);
 ...
 */
export function clockTimeIn(at, timezone) {
...
export function resolveTimeArg(time, timezone, now) {
    if (time === undefined || time.length === 0)
        return clockTimeIn(now(), timezone);
    ...
    if (Number.isNaN(parsed.getTime()))
        throw new Error(`tariff: 非法时间 "${time}"，应为 HH:MM 或 ISO 8601 时间戳`);
